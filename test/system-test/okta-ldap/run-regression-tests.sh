...
# ===========================================================================
section "Phase 12: okta_require_ssl rejects LDAP logins on unencrypted links (#6)"
# ===========================================================================
# Opt-in SSL gate: with okta_require_ssl=true, an LDAP login over an
# unencrypted connection must be rejected ("SSL is required"); with it false
# (default), unencrypted LDAP login still works.
reset_mappings
map_mysql 100 "$ALICE" "$MYSQL_BACKEND"
...
out=$(mysql_ldap "$ALICE" "$ALICE_PASS" "SELECT 1" || true)
if is_query_ok "$out"; then
    pass "MySQL: unencrypted LDAP login works again with okta_require_ssl=false (default)"
else
